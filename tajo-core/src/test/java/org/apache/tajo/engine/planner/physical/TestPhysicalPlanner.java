...
public class TestPhysicalPlanner {
  ...
  public static void setUp() throws Exception {
    ...
    Appender appender = TajoStorageHandler.getFileStorageHandler(conf).getAppender(employeeMeta, employeeSchema,
        employeePath);
    appender.init();
    ...
    appender = TajoStorageHandler.getFileStorageHandler(conf).getAppender(scoreMeta, scoreSchema, scorePath);
    appender.init();
    score = new TableDesc(
  ...
  public static void createLargeScoreTable() throws IOException {
    ...
    Appender appender = TajoStorageHandler.getFileStorageHandler(conf).getAppender(scoreLargeMeta, scoreSchmea,
        scoreLargePath);
  ...
  public final void testCreateScanPlan() throws IOException, PlanningException {
    FileFragment[] frags = TajoStorageHandler.getFileStorageHandler(conf).splitNG(conf, "default.employee", employee.getMeta(),
  ...
  public final void testGroupByPlan() throws IOException, PlanningException {
    FileFragment[] frags = TajoStorageHandler.getFileStorageHandler(conf).splitNG(conf, "default.score", score.getMeta(), score.getPath(),
        Integer.MAX_VALUE);
  ...
  public final void testStorePlan() throws IOException, PlanningException {
    ...
    Scanner scanner = TajoStorageHandler.getFileStorageHandler(conf).getFileScanner(outputMeta, rootNode.getOutSchema(),
        ctx.getOutputPath());
    scanner.init();
    Tuple tuple;
    int i = 0;
  ...
  public final void testStorePlanWithMaxOutputFileSize() throws IOException, PlanningException,
    ...
    FileFragment[] frags = TajoStorageHandler.getFileStorageHandler(conf).splitNG(conf, "default.score_large", largeScore.getMeta(),
    ...
    for (FileStatus status : fs.listStatus(ctx.getOutputPath().getParent())) {
      Scanner scanner = TajoStorageHandler.getFileStorageHandler(conf).getFileScanner(
  ...
  public final void testUnionPlan() throws IOException, PlanningException, CloneNotSupportedException {
    FileFragment[] frags = TajoStorageHandler.getFileStorageHandler(conf).splitNG(conf, "default.employee", employee.getMeta(), employee.getPath(),
        Integer.MAX_VALUE);
  ...
  public final void testCreateIndex() throws IOException, PlanningException {
    ...
    FileSystem fs = workDir.getFileSystem(conf);
    FileStatus [] list = fs.listStatus(StorageUtil.concatPath(workDir, "index"));
    assertEquals(2, list.length);
  }
  ...
  public final void testIndexedStoreExec() throws IOException, PlanningException {
    ...
    SeekableScanner scanner =
        TajoStorageHandler.getSeekableScanner(conf, meta, exec.getSchema(), outputPath);
    scanner.init();
  ...
  public final void testGroupByEnforcer() throws IOException, PlanningException {
    FileFragment[] frags = TajoStorageHandler.getFileStorageHandler(conf).splitNG(conf, "default.score", score.getMeta(), score.getPath(), Integer.MAX_VALUE);
