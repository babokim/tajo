...
public class PhysicalPlannerImpl implements PhysicalPlanner {
      ...
      if (scanNode instanceof PartitionedTableScanNode
        ...
        if (scanNode instanceof PartitionedTableScanNode) {
          if (broadcastFlag) {
            ...
            FileStorageHandler fileStorageHadler = TajoStorageHandler.getFileStorageHandler(ctx.getConf());
            for (Path path : partitionedTableScanNode.getInputPaths()) {
              fileFragments.addAll(TUtil.newList(fileStorageHadler.split(scanNode.getCanonicalName(), path)));
            }
  ...
  public PhysicalExec createIndexScanExec(TaskAttemptContext ctx,
    ...
    Path indexPath = new Path(
        TajoStorageHandler.getFileStorageHandler(ctx.getConf()).getTablePath(annotation.getTableName()), "index");
