...
public class ColumnMapping {
  ...
  public void init() throws IOException {
    String delim = tableMeta.getOption(HBaseStorageHandler.META_ROWKEY_DELIMITER, "").trim();
    if (delim.length() > 0) {
    ...
    for (Pair<String, String> eachMapping: hbaseColumnMappings) {
      ...
      RowKeyMapping rowKeyMapping = HBaseStorageHandler.getRowKeyMapping(cfName, columnName);
  ...
  public static List<Pair<String, String>> parseColumnMapping(TableMeta tableMeta) throws IOException {
    ...
    String columnMapping = tableMeta.getOption(HBaseStorageHandler.META_COLUMNS_KEY, "");
